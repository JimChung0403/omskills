...
- **階段**：Phase 3（核心分析階段，每個 Form 執行一次）
- **觸發方式**：使用者說「分析 {FormName}」或「繼續」（依 Phase 2 建議順序自動選擇下一個 Form）
- **前置條件**：Phase 1a + 1b + Phase 2 已完成
- **重複執行**：依 Form 數量重複，每次分析一個 Form
- **後續 skill**：所有 Form 分析完成後，執行 `/report-generation`（Phase 4）
