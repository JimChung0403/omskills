...
```
你的 VB.NET 專案/
├── CLAUDE.md                          ← Claude Code 自動載入（全域規範）
├── .claude/
│   ├── agents/
│   │   └── analyzer.md                ← Agent 定義（角色、skill 調用、指令對應）
│   └── skills/                        ← 分析技能
...
用 Claude Code 開啟專案，會自動載入 `CLAUDE.md`。

---

## 二、使用方式

你只需要用自然語言或斜線指令告訴 Claude Code 要做什麼，它會自動讀取對應的 SKILL.md 並執行。
...
| `分析 frmOrder` | 執行 `/form-analysis`，以 frmOrder 為目標 |
...
Context window：128K（~131K 個 token）。
...
開新對話**不會丟失進度**。新對話中只需說 `繼續`，Claude Code 會讀取 `memory-bank/progress.md` 自動接續。
...
恢復方式：開啟 Claude Code，說 `目前進度` 或 `繼續`。
...
```
CLAUDE.md                                ← 全域規範（分析深度、報告格式、Memory Bank 機制）
│
├── .claude/
│   ├── agents/
│   │   └── analyzer.md                  ← Agent 定義（角色、skill 調用邏輯、指令對應）
│   └── skills/                          ← Skills（5 個）
